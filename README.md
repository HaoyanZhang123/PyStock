...
## 写给所有看到这个项目的人
本人四个月前还是完全的电脑小白，靠着网上各种免费的教程、书籍、社区、开源软件等学成现在的样子，发自内心的感谢所有愿意将知识免费分享出来的所有人，我也会将这种精神继续传递下去，虽然我现在能做的东西浅显不够成熟，但是我承诺永远会保持开源的态度，毕竟知识属于所有愿意学习的人。

## 注意事项
由于github以及git LFS限制，实际上的stock_daily.sqlite并未被上传，但是可以根据README的内容自行爬取。
本项目最关键的模型回测和选股功能并未开发，由于技术力有限，本人打算结合手动测试，因此以后的版本也许也看不到这些功能，十分抱歉！
本项目为了隐私保护，未上传实际token，请自行注册tushare以使用。
...
- data/: 存储数据文件
  ...
  - stock_daily.sqlite:存了stock_list.csv里所有股票从2023.1.1至今的数据包括：
    ...
    最低价（low）：每日最低价格。
    收盘价（close）：每日收盘价格。
    前收盘价（pre_close）：前一交易日收盘价。
    成交量（vol）：每日成交量（单位：股）。
    成交额（amount）：每日成交额（单位：千元）。
